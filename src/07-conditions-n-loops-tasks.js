...
function isCreditCardNumber(ccn) {
  const strCcn = ccn.toString().split('').reverse();
  return strCcn.map((item, index) => {
    if ((index + 1) % 2 === 0) {
      const newItem = +item * 2;
      if (newItem > 9) {
        return (newItem % 10) + Math.trunc(newItem / 10);
      }
      return newItem;
    }
    return +item;
  }).reduce((sum, item) => sum + item, 0) % 10 === 0;
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    const bracket = str[i];
    if (bracket in brackets && brackets[bracket] === stack[stack.length - 1]) {
      stack.pop();
    } else {
      stack.push(bracket);
    }
...
function getCommonDirectoryPath(pathes) {
  ...
  while (isCommon) {
    ...
    isCommon = pathesArr.every((item) => item[index] === pathesArr[0][index]);
    currentIndex += 1;
  }
  if (currentIndex === 1) return '';
  return `${pathesArr[0].slice(0, currentIndex - 1).join('/')}/`;
}
...
function evaluateTicTacToePosition(position) {
  ...
  if (position[1][1] && ((position[1][0] === position[1][1] && position[1][0] === position[1][2])
    || (position[0][1] === position[1][1] && position[0][1] === position[2][1]))) {
    return position[1][1];
  }

  if (position[2][2] && ((position[2][0] === position[2][1] && position[2][0] === position[2][2])
